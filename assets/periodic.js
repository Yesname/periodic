...
function Element(periodic,number){
	...
	this.inject();
	//alive.animate(this.g, 'svgTransform', 'translate('+this.gx+','+this.gy+')', 5000, Math.random()*1000,'elastic');
}
Element.prototype.inject = function(){
	this.g = document.createElementNS(this.periodic.ns,'g');
	this.gx = this.element.position[1] * this.periodic.cellWidth + this.element.position[1] * this.periodic.gaps;
	...
	this.shortLabel.style.fontSize = this.periodic.cellWidth / 2 + 'px';
	...
	this.label.style.fontSize = this.periodic.cellWidth / 7 + 'px';
	...
	this.index.style.fontSize = this.periodic.cellWidth / 7 + 'px';
	...
	this.atomic.style.fontSize = this.periodic.cellWidth / 7 + 'px';
...
Element.prototype.update = function(mode){
	if (mode == 'full'){
		...
		this.gy = this.element.position[0] * this.periodic.cellHeight + this.element.position[0] * this.periodic.gaps + this.periodic.yPadding;
		alive.animate(this.g, 'svgTransform', 'translate('+this.gx+','+this.gy+'))', 500, Math.random()*50 + 1000 - this.element.index*8, 'soft');	 	
		...
		if ((this.element.index >=57 && this.element.index <=71) || (this.element.index >=89 && this.element.index <=103)){
			...
			alive.animate(this.g, 'svgTransform', 'translate('+this.gx+','+this.gy+'))', 500, Math.random()*50 + this.element.index*4 + 200, 'soft');
			...
			this.gy = this.element.position[0] * this.periodic.cellHeight + this.element.position[0] * this.periodic.gaps + this.periodic.yPadding;
			alive.animate(this.g, 'svgTransform', 'translate('+this.gx+','+this.gy+'))', 500, Math.random()*50 + this.element.index*2, 'soft');
		} else {
			this.gx = this.element.position[1] * this.periodic.cellWidth + this.element.position[1] * this.periodic.gaps  + this.periodic.xPadding;
			this.gy = this.element.position[0] * this.periodic.cellHeight + this.element.position[0] * this.periodic.gaps + this.periodic.yPadding;
			alive.animate(this.g, 'svgTransform', 'translate('+this.gx+','+this.gy+'))', 500, Math.random()*50 + this.element.index*2, 'soft');	
		...
		alive.animate(this.g, 'svgTransform', 'translate('+this.gx+','+this.gy+'))', 500, this.element.index*16, 'soft');	
	}
}
Element.prototype.repaint = function(scheme){
	...
	if (scheme == 'groups'){
		...
		if (this.element.sSAbu){
			...
			this.setData(text, this.periodic.cellWidth / 5);
		} else {
			this.setData(this.element.sn, this.periodic.cellWidth / 2);
		}
	} else if (scheme == 'melting'){
		if (this.element.melt){
			...
			this.setData(Math.round(this.element.melt - 273) + '°', this.periodic.cellWidth / 4);
		} else {
			this.setData(this.element.sn, this.periodic.cellWidth / 2);
		}
		...
		if (this.element.boil){
			...
			this.setData(Math.round(this.element.boil - 273) + '°', this.periodic.cellWidth / 4);
		} else {
			this.setData(this.element.sn, this.periodic.cellWidth / 2);
		}
		...
		if (this.element.elneg){
			...
			this.setData(this.element.elneg, this.periodic.cellWidth / 3);
		} else {
			this.setData(this.element.sn, this.periodic.cellWidth / 2);
		}
...
Element.prototype.setData = function(data,fontSize){
	if (data != this.shortLabel.innerHTML){
		alive.animate(this.shortLabel, 'opacity', 0, 200, Math.random() * 5 * this.element.index, function(){
			...
			this.shortLabel.style.fontSize = fontSize + 'px';
